...
public class CheckoutTaskSetCoupons extends AsyncTask<String, Void, Integer> implements AsyncTaskCode {
	...
	public CheckoutTaskSetCoupons(CheckoutActivity activity, ArrayList<Coupon> coupons) {
	...
	protected Integer doInBackground(String... arg0) {
		...
		try {			
			if(mCoupons==null || mCoupons.size() ==0) {
				...
				for(Coupon mCoupon: mCoupons) {
					client.cartCouponAdd(cartId,mCoupon.getCode());
					
				}
				mActivity.getOrder().setCoupons(mCoupons);				
			}			
		}catch(Exception e) {
			return COUPON_ERROR;
		}		
