...
//	public static ServerSocket m_serverSocket = null;
	...
	{
		((MainApp)getApplicationContext()).matchFirst = true;
        BroadcastReceiver mReceiver = new BroadcastReceiver()
        {
         public void onReceive(Context context, Intent intent)
         {
	       String  Ctrl_Code = intent.getStringExtra("startASR_Command");
           if(Ctrl_Code.equalsIgnoreCase("startASR"))
            {	
        	   
        	  if(((MainApp)getApplicationContext()).matchFirst)
        	  {
        		  ((MainApp)getApplicationContext()).matchFirst =false;
        		  ((MainApp)getApplicationContext()).moderatorSW = !((MainApp)getApplicationContext()).moderatorSW;
        	  }//*/

              if(((MainApp)getApplicationContext()).moderatorSW == true)
               {
                  startASR();            
               }
              else if(((MainApp)getApplicationContext()).moderatorSW == false)
              {    
        ...
        IntentFilter intentFilter = new IntentFilter("android.intent.action.startASR");
...
//								if (mCurrentFocusView != null && (mCurrentFocusView instanceof EditText)) mASRSelectorDialog.showASRSelector(speech2text, (EditText) mCurrentFocusView);
								...
								else
								{
									...
									String strSpeak[] = { "Can you say that again.", " I don't understand.", "I beg your pardon."};
	...
	public boolean dispatchKeyUpEvent(int keyCode, KeyEvent event, View currentFocusView)
	{
		...
		// keycode : 41 是要抓 bluetooth keyboard 的 m 鍵
...
//				strArray[1] += CFG.WEATHER.currentCondition.getDescr() + ", at " + Math.round((CFG.WEATHER.temperature.getTemp() - 275.15)) + "°C";
				strArray[0] += CFG.YAHOOWEATHER.getConditiontext();
				strArray[1] += CFG.YAHOOWEATHER.getConditiontext() + ", at " + CFG.YAHOOWEATHER.getTemperature() + "°C";
...
//				Log.i("processASRCMDAdapter", "===  cmd : " + cmd + " / cmdSubStr : " + cmdSubStr + " / msg : " + msg);
				...
				{
					// for 字串完全比對方式
					System.out.println("compare string ("+msg+") : ("+cmd+")");
