...
import com.kentec.milkbox.grocery.task.GroceryLoadOrderHistoryTask;
...
public class GroceryMainActivity extends ASRActivity
{
	...
	public static final int VOICE_RETURN = 9;
	...
	private LinearLayout cartBtn, searchBtn, wishBtn, historyBtn, voicebtn ;
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		voicebtn = (LinearLayout) findViewById(R.id.voiceBtn) ;
		...
		voicebtn.setOnClickListener(new OnClickListener()
		{
			...
			public void onClick(View v)
			{
				...
				intent.setClass(GroceryMainActivity.this, GroceryVoiceMainActivity.class);
//				startActivity(intent); 
				startActivityForResult(intent,VOICE_RETURN);
			}
		}) ;
		...
		// add Category Title TextView's onClick event to call startASR() on Category Title
		mCategroyTv.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				startASR();
			}	
		});
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data)
	{
		...
		else if (requestCode == GroceryMainActivity.VOICE_RETURN)
		{
			this.finish() ;
		}
	}
...
//		MilkBoxTTS.speak("You can select an item to cancel or add to cart. ", "openWishList");
	...
	public void openSearch()
	{
		...
		if (mKeyword == null)
		{
			...
			*/
			int indexNo = (int) (Math.random() * 2) + 1; // 亂數得到 1 - 2 的值
			moderatorDialog( mActivity.getString(R.string.opensearchdialog), indexNo, 5000);
			MilkBoxTTS.speak(mActivity.getString(R.string.opensearchtts), "searchDialog");	
		}
		else
		{
			mKeyword = null;
...
//		Log.d("andy", "new position: " + newPos);
	...
	*/
	...
	public void openGroceryItem(String strValue) {
		...
		int position = mMainGv.getSelectedItemPosition();
		int itemNumber = Integer.parseInt(strValue);
...
//				"Which number of item do you want to browse?", "echoGroceryStoreCategory");
		MilkBoxTTS.speak("Please select a product or a number.", "echoGroceryStoreCategory");
	}
	 ...
	 * 2014.03.28
	 * 確認目前訂單狀態:
	 * 當end user說"check order status"時
	 *	1. 有on the way訂單: reply "All your orders are arrived" 
	 *	2. 無on the way訂單: reply "Here is the order satus" 畫面顯示所有已下單未到貨的order history
	 */
	
	public void openLoadOrderHistory() {
		dialog = new OrderHistoryDialog(this, mCustomerId, mCartId, mStoreId);
		new GroceryLoadOrderHistoryTask(mActivity, orderHistoryLoadSuccessListener, mCustomerId, mStoreId);
	}
	
	GroceryLoadOrderHistoryTask.OnSucessListener orderHistoryLoadSuccessListener = new GroceryLoadOrderHistoryTask.OnSucessListener() {
		
		@Override
		public void success(ArrayList<OrderHistory> mList) {
			int indexNo = (int) (Math.random() * 2) + 1; 
			
			Boolean FlagForOrderFinished=true;
			for(int i=0; i<mList.size(); i++) {
				if( ! "complete".equalsIgnoreCase(mList.get(i).getStatus())) {
					FlagForOrderFinished=false;
					break;
				}
			}
			if( FlagForOrderFinished == true ) {
				moderatorDialog( "All your orders are arrived.", indexNo, 6000);
				MilkBoxTTS.speak("All your orders are arrived.", "echoCheckOrderStatus");
			}else {
				MilkBoxTTS.speak("Here is the order satus.", "echoCheckOrderStatus");
				dialog.show();
				reTimeOut();
			}
	...
	GrocerySearchAddToCartTask.OnSucessListener addProductSearchSuccessListener = new GrocerySearchAddToCartTask.OnSucessListener() {
		...
		public void success(ProductResult mProductResult) {
			...
			int indexNo = (int) (Math.random() * 2) + 1;
			if(pList.size() > 0) {
				moderatorDialog( "Product ["+pList.get(0).getName()+"] already added to cart。"
								, indexNo, 5000);
				MilkBoxTTS.speak("I already added  "+pList.get(0).getName()+" to cart。"
		...
		public void fail(int failCode) {
			int indexNo = (int) (Math.random() * 2) + 1;
			moderatorDialog( "I can not find this item."
					, indexNo, 4000);
			MilkBoxTTS.speak("I can not find this item."
					, "voicesearchaddtocart");
	...
	GrocerySearchDeleteFromCartTask.OnSucessListener deleteProductSearchSuccessListener = new GrocerySearchDeleteFromCartTask.OnSucessListener() {
		...
		public void success(String productName) {
			int indexNo = (int) (Math.random() * 2) + 1;
			moderatorDialog( "Product ["+productName+"] already removed from cart。"
					, indexNo, 5000);
			MilkBoxTTS.speak("I already removed "+productName+"  from cart."
		...
		public void fail(int failCode) {
			int indexNo = (int) (Math.random() * 2) + 1;
			moderatorDialog( "Can not find this product from cart."
					, indexNo, 4000);
			MilkBoxTTS.speak("Can not find this product from cart."
					, "voicesearchdeletefromcart");
	...
	GroceryCategoryTask.OnSucessListener categorySuccessListener = new GroceryCategoryTask.OnSucessListener()
	{
		...
		public void success(final ArrayList<Category> cList)
		{
			...
			if(flagForGreetingCommand == true) {
				moderatorDialog( "Welcome to Grocery store.\n" +
						"There are "+cList.size()+" categories to choose from.\n" +
//						"Which number of category do you want to browse?\n" +
						"Please select a category or a number.\n"+
						"You can say <Page Down> & <Page Up> to change pages.", indexNo, 9000);
...
//				Log.d("andy","" + list.length );
		...
		protected void onPostExecute(Integer result) {
			if (mListLenght == 0)
				mcartTextView.setText( getResources().getString(R.string.checkout) );
			else
				mcartTextView.setText(
					getResources().getString(R.string.checkout) + "(" + mListLenght + ")" );
