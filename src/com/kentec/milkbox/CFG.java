...
{	
	...
	public static YahooWeather YAHOOWEATHER = null; // for Moderator use, 每當系統讀取完目前天氣候會保存一份至此以供 moderator 回應天氣狀況時使用	
	...
	{
		host = (DEBUG.TESTMAGENTO) ? "http://54.187.180.50/boxen" : host;
		host = ((host == null) || (host.length() <= 0)) ? new String("http://54.187.180.50/boxen") : host;
		HOST = host;
